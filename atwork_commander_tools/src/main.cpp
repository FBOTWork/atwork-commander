...
/**
 * @brief subscriber callback which converts the message to a human readable
 * format and write a csv file
 * @param[in] ObjectTask message
 */
void objectTaskCallback(
    const atwork_commander_msgs::ObjectTask::ConstPtr &msg) {
  ...
  for (int i = 0; i < numberOfSubtasks; i++) {
    std::string object(
        atwork_commander_msgs::objectName(msg->subtasks[i].object.object));
    std::string placementTarget(
        atwork_commander_msgs::objectName(msg->subtasks[i].object.target));
    ROS_INFO_STREAM("[TASK_OUTPUT] object " + object + " from workstation " +
                    msg->subtasks[i].source + " to workstation " +
                    msg->subtasks[i].destination +
                    (" in placement target " + placementTarget));
    write2File(filename, object + "," + msg->subtasks[i].source + "," +
                             msg->subtasks[i].destination + "," +
                             placementTarget + "\n");
  }
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subObjectTask =
      nh.subscribe("/atwork_commander/object_task", 1000, objectTaskCallback);
